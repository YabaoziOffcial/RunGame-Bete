...
public class RecyclingListView : MonoBehaviour
{
    ...
    public virtual int RowCount
    {
        ...
        set
        {
            ...
            {
                rowCount = value;
                CountChanageCallBack();
            }
        }
    }
    // 列表数目发生改变改变需要重新刷新高度
    public virtual void CountChanageCallBack()
    {
        itemHeights = new float[rowCount];
        prefixHeights = new float[rowCount];

        for(int i = 0; i < rowCount; i++)
        {
            UpdateChild(ChildObj, i);
            itemHeights[i] = ChildObj.GetPerferenceHeight();
            Debug.Log(itemHeights[i]);
            if(i > 0)
            {
                prefixHeights[i] = itemHeights[i] + prefixHeights[i-1];
            }
        }
        totalContentHeight = prefixHeights[rowCount - 1];
        
        // 先禁用滚动变化
        ignoreScrollChange = true;
        UpdateContentHeight(); // 更新高度
        // 重新启用滚动变化
        ReorganiseContent(true); // 重新计算item
    ...
    protected float[] itemHeights;
    protected float[] prefixHeights;
    ...
    protected bool ignoreScrollChange = false; // 忽略滚动变化
    ...
    /// <param name="row">行号</param>
    /// <returns></returns>
    public virtual float GetRowScrollPosition(int row, ScrollPosType posType)
    {
        // 视图高
    ...
    /// </summary>
    /// <param name="row">行号</param>
    protected virtual RecyclingListViewItem GetRowItem(int row)
    {
        if(childItems != null &&
    ...
    protected virtual int WrapChildIndex(int idx)
    {
        while(idx < 0)
    ...
    protected virtual void UpdateChild(RecyclingListViewItem child, int rowIdx)
    {
        ...
        } else
        {
            ...
            child.NotifyCurrentAssignment(this, rowIdx);    // 切换行
            // 更新数据
    ...
    protected virtual void UpdateContentHeight()
    {
        ...
        scrollRect.content.sizeDelta = new Vector2(sz.x, totalContentHeight);
    }
